...
const stats = [
  ...
  { value: 95, suffix: '%', label: 'Precisión voto automatizado', icon: Shield },
  { value: 500, suffix: '+', label: 'Empresas en gobernanza', icon: Users },
]
    ...
    <section id="platform" className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden px-6 pb-16 pt-28">
      ...
      <div className="relative z-10 mx-auto max-w-7xl text-center">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass mb-8 inline-flex items-center gap-2 rounded-full border border-acres-cyan/20 px-4 py-2">
          ...
          <span className="font-mono text-xs uppercase tracking-widest text-acres-cyan">Gobernanza IA para juntas virtuales</span>
        </motion.div>
        ...
        <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="mb-6 font-display text-5xl font-800 leading-none tracking-tight text-white md:text-7xl lg:text-8xl">
          ACRES
          <br />
          <span className="gradient-text-cyan">Gobernanza Corporativa</span>
          <br />
          <span className="text-4xl text-slate-300 md:text-5xl lg:text-6xl">asistida por Inteligencia Artificial</span>
        </motion.h1>
        ...
        <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="mx-auto mb-10 max-w-3xl text-lg leading-relaxed text-slate-400 md:text-xl">
          Plataforma para automatizar asistencia, quórum, conteo de votos, transcripción, acuerdos y borradores legales en juntas de accionistas virtuales por Google Meet.
        </motion.p>
